...
public class Serpent {
    ...
    public void generateRoundKeys(int w[]) {
        ...
        for (int i = 0; i < 33; i++) {
            for (int k = i * 4; k < i * 4 + 4; k++) {
                var key_left = permutationBits(new byte[]{(byte) (w[k] >>> 24 & 0xff), (byte) (w[k] >>> 16 & 0x00ff)}, Entities.forSerpent.sBoxTable[j]);
                var key_right = permutationBits(new byte[]{(byte) (w[k] >>> 8 & 0x0000ff), (byte) (w[k] & 0x000000ff)}, Entities.forSerpent.sBoxTable[j]);
                keys[k][0] = (key_left[0]);
                keys[k][1] = (key_left[1]);
    ...
    public byte[] decrypt(byte[] input) {
        input = permutationBits(input, Entities.forSerpent.IP);
        for (int i = 31; i >= 0; i--) {
            if (i == 31) {
                for (int j = 0; j < 16; j++) {
        ...
        input = permutationBits(input, Entities.forSerpent.FP);
        return input;
    }
    ...
    public byte replaceWithSBox(byte b, int i) {
        return (byte) Entities.forSerpent.sBoxTable[i][b];
    }

    public byte replaceWithInverseSBox(byte b, int i) {
        return (byte) Entities.forSerpent.sBoxInverseTable[i][b];
    }
